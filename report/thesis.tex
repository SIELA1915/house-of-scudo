...
\begin{abstract}
  Heap exploits are a very frequent issue in C programming, due to the quite high
  complexity of memory management for programmers. Attackers can use such heap exploits
  like overflows, double free's and similar to abuse the underlying mechanics of the heap
  allocator and get even bigger control over heap memory such as arbitrary writes. However
  Scudo is an allocator used on Android 11+ that is supposed to be specifically hardened
  to prevent or at least complicate such attacks.
  ...
  Scudo allocator that is exploited in both of our exploits. We also show that the impact
